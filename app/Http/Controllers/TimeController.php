...
class TimeController extends Controller
{
    ...
    function setStartTime($sorce_id ,$day ,$type , $start_time)
    {
        $time= time::find($sorce_id,$day,$type);
        $time -> start_time = $start_time->input('$start_time');
        $time -> update();
       
        return $time;
    }

    public function editTime(Request $req,$id)
    {
        $time= time::find($id);
        $time->start_time =$req->input('start_time');
        $time->end_time =$req->input('end_time');
    ...
    function setEndtTime($sorce_id ,$day ,$type , $end_time)
    {
        $time= Time::find($sorce_id,$day ,$type);
        $time -> end_time = $end_time->input('$end_time');
        $time -> update();
